...
| `NOT_WIRED` | **12** entries | Includes Phase 160 classification of blocked `new-deal-create`; both PR 105 fields are now closed — `origination-loan-structure-fields` closed by Factory Arc Phase 3 (loan purpose/term/ownership now write/read via updateDealProfile.ts against the live PR105 columns), `financial-spread-persistence` closed by Factory Arc Phase 4 (Global Cash Flow figures now write/read via updateDealProfile.ts against the live PR105 `cr664_financialspreadinputs` column); +1 from PR 106 (`risk-rating-persistence`); +2 from PR 107 (`closing-document-persistence`, `funding-authorization-persistence`) — schema-pending entries see docs/factory-arc/PR105_LOAN_STRUCTURE_SCHEMA_MIGRATION.md, docs/factory-arc/PR106_RISK_RATING_SCHEMA_MIGRATION.md, docs/factory-arc/PR107_CLOSING_FUNDING_ACTIVATION.md |
