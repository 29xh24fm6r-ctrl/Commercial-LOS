...
| `GOVERNED_WRITES` | **23** entries | +2 since Phase 110 (Phase 105 `deal-borrower-update-email`, Phase 160 `deal-log-activity`); +1 since (Phase 237 `deal-stage-advance`, forward stage-advance write — legacyDisciplineExempt from the Phase 46/47/49/50 sweeps, see platformInventory.ts); +6 from the Final LOS Completion arc (Workstream M) registering the durable-record governed writes Workstreams C/D/E/F/H/J shipped (Credit Approval Decision, Commitment, Condition Verification, Executed Document Attestation, Booking QC, Adverse Action Record) — a registry blind spot those workstreams had until now, not new gaps; +1 from Workstream O (`data-quality-flag-create`, the first write that CREATES a data quality flag); +2 from the 146 Factory arc (`credit-memo-finalize` — Workstream 146-B, `assign-servicing-owner` — Workstream 146-E) |
| `LOCAL_ONLY_FLOWS` | **16** entries | +1 (Phase 105 updated `borrower-update-draft` shape; net inventory shifted) |
| `NOT_WIRED` | **14** entries | Includes Phase 160 classification of blocked `new-deal-create`; all three of PR 105/106's original schema-pending fields are now closed — `origination-loan-structure-fields` closed by Factory Arc Phase 3 (loan purpose/term/ownership now write/read via updateDealProfile.ts against the live PR105 columns), `financial-spread-persistence` closed by Factory Arc Phase 4 (Global Cash Flow figures now write/read via updateDealProfile.ts against the live PR105 `cr664_financialspreadinputs` column), `risk-rating-persistence` closed by Factory Arc Phase 5 (risk rating / underwriting recommendation now write/read via updateDealProfile.ts against the live PR106 columns); +2 from PR 107 (`closing-document-persistence`, `funding-authorization-persistence`); +2 from Factory Arc Phase 14 (`annual-review-persistence`, `portfolio-boarding-audit-governance` — a registry blind spot this domain had until now, not new gaps); +1 from Workstream R (`portfolio-migration-reconciliation` — the reconciliation engine exists and is tested, only the migration-control schema is missing) — schema-pending entries see docs/factory-arc/PR105_LOAN_STRUCTURE_SCHEMA_MIGRATION.md, docs/factory-arc/PR107_CLOSING_FUNDING_ACTIVATION.md, docs/factory-arc/PR126_PORTFOLIO_SERVICING_COMPLETION.md |
