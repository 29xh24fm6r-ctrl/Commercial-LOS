...
- **Current state.** **Partially operational (advanced by Phase 80).** **Phase 80** shipped Deal Autopilot Lite — a deterministic Next Best Actions panel on the Banker Deal Workspace. The panel surfaces at most 3 priority-ordered suggestions (HIGH / MEDIUM / LOW) derived from existing signals: overdue tasks, pending-review documents past 7 days, closing-soon + stale activity, stage-aging ≥30 days, outstanding documents, Phase 73 memo consistency findings, draft memos without findings, stale activity ≥14 days. Every suggestion carries `isAutomated: false` at the type level — the panel never creates a task, never sends an email, never advances the stage, never marks documents reviewed, never calls AI, never writes to Dataverse. Each "Open …" button scrolls the relevant card into view; the banker still chooses what to do. Conservative copy ("Autopilot suggests, banker decides. Derived from current deal records. Nothing happens automatically.") is statically enforced against AI / autopilot-executed / approved / decisioned / guaranteed / "system will complete" / prediction vocabulary.
