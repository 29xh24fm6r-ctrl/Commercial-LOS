...
- **Current state.** **Operational.** Phase 4 banker workspace + Phase 54 pending-review signal + Phase 56 (work queue refresh) + MyWorkQueue surfaces. **Phase 75** added a deterministic personal activity/workload snapshot card (PersonalActivitySummary). **Phase 76** added a deterministic Relationship Memory Lite card (RelationshipMemory). **Phase 78** layered a local-only "Draft relationship note" button on each client row in the RelationshipMemory card — the banker drafts a note + optional follow-up + optional open-asks block, then copies the formatted preview for their own system of record. No Dataverse write, no audit, no timeline — inventoried as `LOCAL_ONLY_FLOWS.relationship-note-draft`. None of Phase 75/76/78 introduces a new query shape or new write surface.
...
- **Current state.** **Partially operational (advanced by Phase 80 + Phase 81).** **Phase 80** shipped Deal Autopilot Lite — a deterministic Next Best Actions panel on the Banker Deal Workspace (8 signals, top 3 surface per deal, every suggestion typed `isAutomated: false`). **Phase 81** extended the same derivation to the Manager Command Center as a team-level rollup: `deriveManagerAutopilotRollup` invokes Phase 80's `deriveNextBestActions` once per team-pipeline deal, aggregates HIGH/MEDIUM/LOW deal counts, and ranks the top 5 deals by priority → suggestion-count → nearest target close → name. Manager rollup signal coverage is intentionally narrower than the banker panel — it uses deal-record fields only (closing-soon, stage-aging, modifiedon staleness) because `TeamDeal` does not carry per-deal tasks / documents / memos; the card disclaimer states this honestly. No write, no audit, no AI. Conservative copy ("Derived from current records. Nothing happens automatically. No AI or automated decisions.") is statically enforced across both surfaces.
...
- **Current state.** **Partially operational** (advanced by Phase 72 + Phase 80). Phase 25+ timeline ledger exists. Phase 72 shipped a rule-based "activity since last visit" surface on the Activity Timeline card: per-deal local-browser marker + deterministic derivation; a "N new since your last visit" subtitle + per-row "New" badge. Local-only (`LOCAL_ONLY_FLOWS.activity-since-last-visit`); no Dataverse write; no AI; no cross-device sync. **Phase 80** added a stale-activity signal to the autopilot panel — when a deal's most-recent timeline event is more than 14 days old (or absent entirely), the panel surfaces a LOW priority "consider a borrower check-in" suggestion. Still deterministic, still read-only.
