...
- **Current state.** **Partially operational (advanced by Phase 80 + Phase 81 + Phase 82).** **Phase 80** shipped the per-deal Next Best Actions panel on the Banker Deal Workspace (8 signals; top 3 per deal; `isAutomated: false` typed). **Phase 81** extended the same derivation to a team-level rollup on the Manager Command Center (3–4 signals fire because `TeamDeal` carries deal-record fields only). **Phase 82** added the banker-side counterpart: `<BankerAutopilotRollup />` on the Banker Command Center via `deriveBankerAutopilotRollup`. The banker rollup has RICHER coverage than the manager rollup — 7 of 8 Phase 80 signals fire (overdue-tasks, pending-review-documents, closing-soon-stale-activity, closing-soon, stage-aging, outstanding-documents, draft-memo, stale-activity) because `loadBankerWorkQueueData` already loads tasks / documents / memos. Only `memo-consistency-findings` is silent on the rollup (requires per-deal CreditMemoData with sections, available only on the Deal Workspace panel). Autopilot capability now spans three surfaces — per-deal (banker), team rollup (manager), personal rollup (banker). No write, no audit, no AI on any of the three.
...
- **Current state.** **Partially operational** (advanced by Phase 72 + Phase 80 + Phase 82). Phase 25+ timeline ledger exists. Phase 72 shipped a rule-based "activity since last visit" surface on the Activity Timeline card. **Phase 80** added a stale-activity signal to the per-deal autopilot panel (using `activity[0]?.eventAt`). **Phase 82** extends the same signal to the personal banker rollup via `deal.lastActivityOn` (modifiedon) as the activity proxy — the same coarse proxy Phase 81 uses on the manager rollup. The banker therefore sees "no recent activity" signals AT BOTH the cross-deal rollup level AND the per-deal panel level. Still deterministic, still read-only.
