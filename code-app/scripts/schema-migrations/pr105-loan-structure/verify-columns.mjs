#!/usr/bin/env node
// Verifies whether the PR 105 loan-structure columns exist on cr664_loandeal.
...
async function main() {
  ...
  if (missing.length === 0 && mismatched.length === 0) {
    console.log(`All ${COLUMNS.length} PR 105 loan-structure columns are present on ${TABLE_LOGICAL_NAME}.`);
    process.exit(0);
  }
  if (missing.length > 0) {
