#!/usr/bin/env node
// Verifies whether the PR 142 (N-17) governed test/production classification column exists on
// cr664_loandeal. Exits 0 (present) or 1 (missing/mismatched), printing exactly which.
//
...
async function main() {
  ...
  if (missing.length === 0 && mismatched.length === 0) {
    console.log(`All ${COLUMNS.length} PR 142 test-record-classification column(s) are present on ${TABLE_LOGICAL_NAME}.`);
    process.exit(0);
  }
  if (missing.length > 0) {
