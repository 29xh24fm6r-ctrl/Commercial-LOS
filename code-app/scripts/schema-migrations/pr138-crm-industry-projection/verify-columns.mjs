#!/usr/bin/env node
// Verifies whether the PR 138 CRM industry projection column exists on cr664_loandeal.
// Exits 0 (present) or 1 (missing), printing which.
//
...
async function main() {
  ...
  if (missing.length === 0 && mismatched.length === 0) {
    console.log(`All ${COLUMNS.length} PR 138 CRM industry projection columns are present on ${TABLE_LOGICAL_NAME}.`);
    process.exit(0);
  }
  if (missing.length > 0) {
