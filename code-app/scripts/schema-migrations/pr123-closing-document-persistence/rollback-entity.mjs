#!/usr/bin/env node
// Rolls back the cr664_closingdocumentmanifest table by deleting the ENTIRE
// entity (all its columns and any rows go with it). Safe at any point
// before a live persistence adapter is wired -- no live code writes to this
// table until then (createInMemoryClosingDocumentStore() is still the
// mounted store as of Factory Arc Phase 11). If real closing-document
// manifests have already been recorded, export them first: deleting the
// entity deletes that data permanently.
//
