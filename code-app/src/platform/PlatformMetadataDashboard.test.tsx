...
describe('Phase 142B — platform metadata dashboard', () => {
  ...
  it('renders workspace capability groups and next phases', () => {
    ...
    expect(screen.getByText(/Next recommended platform phases/i)).toBeInTheDocument();
  });
  ...
  it('renders without workflow routing data (optional prop)', () => {
    const { container } = render(<PlatformMetadataDashboard context={{ workspace: 'strategy' }} />);
    expect(container.textContent ?? '').not.toContain('Workflow routing');
  });
  ...
  it('renders without product/process template data (optional prop)', () => {
    const { container } = render(<PlatformMetadataDashboard context={{ workspace: 'strategy' }} />);
    expect(container.textContent ?? '').not.toContain('Product / process templates');
  });
  ...
  it('renders the product/process template summary when provided (no mutation controls)', () => {
    ...
    expect(screen.getByText(/^Product \/ process templates$/)).toBeInTheDocument();
  ...
  it('renders without servicing lifecycle data (optional prop)', () => {
    const { container } = render(<PlatformMetadataDashboard context={{ workspace: 'strategy' }} />);
    expect(container.textContent ?? '').not.toContain('Servicing lifecycle');
  });
  ...
  it('renders the servicing lifecycle summary when provided (no mutation controls)', () => {
    ...
    expect(screen.getByText(/^Servicing lifecycle$/)).toBeInTheDocument();
  ...
  it('renders without integration readiness data (optional prop)', () => {
    const { container } = render(<PlatformMetadataDashboard context={{ workspace: 'strategy' }} />);
    expect(container.textContent ?? '').not.toContain('Integration readiness');
  });
  ...
  it('renders without admin configuration data (optional prop)', () => {
    render(<PlatformMetadataDashboard context={{ workspace: 'strategy' }} />);
    // Exact match: the recommended-phases backlog legitimately contains the phrase
    // "Admin configuration review queue..." as one of its items, so a substring
    // check would false-fail — only the section HEADING (exact text) must be absent.
    expect(screen.queryByText(/^Admin configuration review$/)).not.toBeInTheDocument();
  });
  ...
  it('renders the admin configuration summary when provided (no apply controls)', () => {
    ...
    expect(screen.getByText(/^Admin configuration review$/)).toBeInTheDocument();
