 ...
 * mid-load, or — as observed in CI — a test unmounts the tree while `listVerificationsForDeal` is
...
const listVerificationsForDealMock = vi.fn();
vi.mock('../documentation/conditionVerificationStore', () => ({
  createDataverseConditionVerificationStore: () => ({
    listVerificationsForDeal: (...args: unknown[]) => listVerificationsForDealMock(...args),
...
describe('DealConditionVerificationPanel — async loader is unmount-safe', () => {
  ...
  beforeEach(() => {
    unhandledRejections = [];
    listVerificationsForDealMock.mockReset();
  ...
  it('unmount before the load resolves causes no state update and no unhandled rejection', async () => {
    ...
    listVerificationsForDealMock.mockReturnValue(pending);

    const { unmount } = render(<DealConditionVerificationPanel {...baseProps()} />);
    expect(await screen.findByText(/loading condition verifications/i)).toBeInTheDocument();
  ...
  it('unmount before the load rejects causes no unhandled rejection', async () => {
    ...
    await withTornDownWindow(async () => {
      rejectList(new Error('OData-Error: cr664_conditionverification internal trace'));
      await Promise.resolve();
      await Promise.resolve();
      await Promise.resolve();
    });
  ...
  it('a rejection while still mounted still displays the mapped business-safe error, never the raw message', async () => {
    listVerificationsForDealMock.mockRejectedValue(new Error('OData-Error: cr664_conditionverification internal trace'));

    render(<DealConditionVerificationPanel {...baseProps()} />);
    ...
    expect(screen.queryByText(/cr664_conditionverification/)).toBeNull();
  });
});
