...
export type EnsureDealFolderOutcome = { readonly kind: 'ready'; readonly folder: DealSharePointFolderIdentity; readonly created: boolean } | { readonly kind: 'dry_run' | 'blocked'; readonly reason: string };
export async function ensureDealSharePointFolder(input: { readonly mode: DealDocumentStorageMode; readonly authorized: boolean; readonly request: DealSharePointFolderRequest }, deps: { readonly connector: DealSharePointDocumentPort; readonly persistence: DealDocumentStoragePersistence }): Promise<EnsureDealFolderOutcome> {
  ...
  if (persisted) {
    if (persisted.borrowerIdentity !== input.request.borrowerIdentity) return { kind: 'blocked', reason: 'Persisted folder borrower identity does not match the deal.' };
  ...
  if (input.mode === 'DRY_RUN') return { kind: 'dry_run', reason: 'DRY_RUN validated folder intent; no folder or Dataverse identity was created.' };
