 ...
 * mid-load, or — as observed in CI — a test unmounts the tree while `listAttestationsForDeal` is
...
const listAttestationsForDealMock = vi.fn();
vi.mock('../closing/executedDocumentAttestationStore', () => ({
  createDataverseExecutedDocumentAttestationStore: () => ({
    listAttestationsForDeal: (...args: unknown[]) => listAttestationsForDealMock(...args),
    createAttestationRecord: vi.fn(),
  }),
}));
...
describe('DealExecutedDocumentAttestationPanel — async loader is unmount-safe', () => {
  ...
  beforeEach(() => {
    unhandledRejections = [];
    listAttestationsForDealMock.mockReset();
  ...
  it('unmount before the load resolves causes no state update and no unhandled rejection', async () => {
    ...
    listAttestationsForDealMock.mockReturnValue(pending);

    const { unmount } = render(<DealExecutedDocumentAttestationPanel {...baseProps()} />);
    expect(await screen.findByText(/loading executed document attestations/i)).toBeInTheDocument();
  ...
  it('unmount before the load rejects causes no unhandled rejection', async () => {
    ...
    await withTornDownWindow(async () => {
      rejectList(new Error('OData-Error: cr664_executeddocattestation internal trace'));
      await Promise.resolve();
      await Promise.resolve();
      await Promise.resolve();
    });
  ...
  it('a rejection while still mounted still displays the mapped business-safe error, never the raw message', async () => {
    listAttestationsForDealMock.mockRejectedValue(new Error('OData-Error: cr664_executeddocattestation internal trace'));

    render(<DealExecutedDocumentAttestationPanel {...baseProps()} />);
    ...
    expect(screen.queryByText(/cr664_executeddocattestation/)).toBeNull();
  });
});
