 ...
 * mid-load, or — as observed in CI — a test unmounts the tree while `listCommitmentsForDeal` is
...
const listCommitmentsForDealMock = vi.fn();
vi.mock('../commitment/commitmentRecordStore', () => ({
  createDataverseCommitmentStore: () => ({
    listCommitmentsForDeal: (...args: unknown[]) => listCommitmentsForDealMock(...args),
    createCommitmentRecord: vi.fn(),
    respondToCommitment: vi.fn(),
  }),
}));
...
describe('DealCommitmentPanel — async loader is unmount-safe', () => {
  ...
  beforeEach(() => {
    unhandledRejections = [];
    listCommitmentsForDealMock.mockReset();
  ...
  it('unmount before the load resolves causes no state update and no unhandled rejection', async () => {
    ...
    listCommitmentsForDealMock.mockReturnValue(pending);

    const { unmount } = render(<DealCommitmentPanel {...baseProps()} />);
    expect(await screen.findByText(/loading commitment records/i)).toBeInTheDocument();

    unmount();
    await withTornDownWindow(async () => {
      resolveList({ success: true, commitments: [] });
  ...
  it('unmount before the load rejects causes no unhandled rejection', async () => {
    ...
    await withTornDownWindow(async () => {
      rejectList(new Error('OData-Error: cr664_commitmentrecord internal trace'));
      await Promise.resolve();
      await Promise.resolve();
      await Promise.resolve();
    });
  ...
  it('a rejection while still mounted still displays the mapped business-safe error, never the raw message', async () => {
    listCommitmentsForDealMock.mockRejectedValue(new Error('OData-Error: cr664_commitmentrecord internal trace'));

    render(<DealCommitmentPanel {...baseProps()} />);
    ...
    expect(screen.queryByText(/cr664_commitmentrecord/)).toBeNull();
  });
});
