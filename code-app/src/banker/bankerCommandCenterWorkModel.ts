...
export function deriveBankerPipelineByStage(deals: readonly PipelineDeal[]): PipelineSnapshot {
  ...
  for (const d of active) {
    // Speak the canonical stage vocabulary (the reconciliation's ratified names): a stored code
    // or legacy name that maps to a canonical stage displays as its ratified NAME; anything with no
    // clean canonical equivalent keeps its raw value (honest "unmapped"); empty → "Unstaged".
    const canonical = recognizeCanonicalStage(d.stage);
    const stage = canonical ? canonical.name : (d.stage ?? '').trim() || 'Unstaged';
