...
export const PLATFORM_OPERATIONS_CAPABILITY_SPECS: readonly PlatformOperationsCapabilitySpec[] = [
  ...
  },
  {
    ...
    routeState: 'Mounted through the internal CRM Hub; external CRM synchronization is not implied.',
    diState: 'Governed internal CRM adapter and writeback spine are enabled against the verified Dataverse schema.',
    actorAuthorizationRequirement: 'Resolved actor identity + CRM write authorization.',
    auditSinkState: 'Writes audit through the cr664_AuditEvent sink.',
    rollback: 'Set CRM_LIVE_PERSISTENCE_ENABLED = false in src/crm/crmFeatureFlags.ts.',
  },
  {
  ...
  },
  {
    ...
    routeState: 'Mounted — governed portfolio boarding route is enabled.',
    diState: 'Live Dataverse boarding adapter is wired behind verified schema and operator authorization.',
    actorAuthorizationRequirement: 'Resolved actor identity + boarding authorization.',
    auditSinkState: 'Writes audit through the cr664_AuditEvent sink.',
    rollback:
      'Set PORTFOLIO_BOARDING_LIVE_PERSISTENCE_ENABLED / PORTFOLIO_BOARDING_ROUTE_ENABLED = false in src/portfolioBoarding/portfolioLoanBoardingFeatureFlags.ts.',
  },
  {
    key: 'document-upload',
