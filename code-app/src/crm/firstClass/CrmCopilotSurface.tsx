...
export function CrmCopilotSurface({ data, role, userName }: { data: CrmWorkspaceData; role: CopilotWorkspaceContext['workspaceRole']; userName: string }) {
  if (!isCopilotSurfaceLive()) return null;
  const context = buildCrmCopilotContext(data, role, userName);
  const proposedActions = getCopilotConnector().assistWorkspace({
    workspace: context.workspace,
    topBlockers: [],
  }).proposed_actions;
  return <section className="crmws__copilot">
    <CopilotAssistPanel surface="workspace" workspaceContext={context.workspace} proposedActions={proposedActions} />
