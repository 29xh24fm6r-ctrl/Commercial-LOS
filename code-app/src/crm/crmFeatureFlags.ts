/**
 * Phase 141L Ã¢â‚¬â€ CRM feature flags.
 *
 * Gates the first real app-runtime write capability for the CRM Relationship
 * Master. Flags are resolved from an injected config object only Ã¢â‚¬â€ never from an
 * environment secret in client code Ã¢â‚¬â€ and they FAIL CLOSED: a capability is
...
export function deriveCrmFeatureFlagState(
  ...
  const livePersistence = config?.livePersistenceEnabled !== false;
  return {
    // Route registration is intentionally not enabled in Phase 141L.
    CRM_ROUTE_ENABLED: config?.routeEnabled !== false,
    CRM_LIVE_PERSISTENCE_ENABLED: livePersistence,
    CRM_CONTACT_EDITING_ENABLED:
      livePersistence && config?.contactEditingEnabled !== false,
    CRM_VENDOR_EDITING_ENABLED:
      livePersistence && config?.vendorEditingEnabled !== false,
    CRM_TIMELINE_ENABLED: livePersistence && config?.timelineEnabled !== false,
    CRM_ANNUAL_REVIEW_INTEGRATION_ENABLED:
      config?.annualReviewIntegrationEnabled !== false,
  };
}
