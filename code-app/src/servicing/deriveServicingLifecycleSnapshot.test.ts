...
import type { ServicingExceptionStatus, ServicingLifecycleInput } from './servicingLifecycleTypes';
...
describe('Phase 142E — lifecycle snapshot', () => {
  ...
  it('an unreadable servicing-exception source (status "unknown") is NOT treated as healthy', () => {
    const unknownExceptionStatus: ServicingExceptionStatus = {
      status: 'unknown',
      openExceptions: [],
      blockers: [{ code: 'exceptions_unavailable', message: 'Servicing exception records could not be read.' }],
      warnings: [],
      nextBestAction: { code: 'retry_exceptions', label: 'Retry loading servicing exceptions.' },
    };
    const input: ServicingLifecycleInput = { lifecycleId: 'L1', asOfDate: AS_OF, boardedLoan: { verified: true } };
    const stage = deriveServicingLifecycleStage(input);
    ...
    const snapshot = deriveServicingLifecycleSnapshot({
      ...
      exceptionStatus: unknownExceptionStatus,
    });
    expect(snapshot.lifecycleStatus).not.toBe('healthy');
    expect(snapshot.lifecycleStatus).toBe('review_required');
  });
});
